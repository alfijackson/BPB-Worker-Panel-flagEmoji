...
async function getWarpConfigs(request: Request, env: Env): Promise<Response> {
    ...
    try {
        ...
        if (!warpAccounts || warpAccounts.length === 0) {
            return respond(false, HttpStatus.INTERNAL_SERVER_ERROR, 'No Warp accounts available. Please update Warp configs first.');
        }
